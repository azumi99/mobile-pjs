...
import {getRequest, getRequestAll} from '@services/Request/services';
...
const HistoryScreen = () => {
    ...
    try {
      const response =
        user?.role === 'superadmin'
          ? await getHistorytAllSuper(pageNumber, errorFunc)
          : await getHistory(pageNumber, errorFunc, user?.id);
      if (response?.status) {
  ...
  const handleSearch = async (value: string) => {
    ...
    const responseall = await getHistorytAll(errorFunc, user?.id);
    if (value === '') {
      getRequestFunc();
    } else {
